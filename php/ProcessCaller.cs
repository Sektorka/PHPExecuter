...
namespace php
{
    ...
    public delegate void BytesReceivedHandler(object sender,
        BytesReceivedEventArgs e);
    ...
    public class BytesReceivedEventArgs : EventArgs
    {
        ...
        /// </summary>
        /// <param name="text">The text that was received for this event to be triggered.</param>
        public BytesReceivedEventArgs(byte[] text, int length)
        {
            Text = text;
            Length = length;
        }
    ...
    public class ProcessCaller : AsyncOperation
    {
        ...
        public event BytesReceivedHandler BytesOutReceived;

        /// <summary>
        /// Fired for every line of stdErr received.
        /// </summary>
        public event DataReceivedHandler StdErrReceived;
        public event BytesReceivedHandler BytesErrReceived;
        ...
        protected virtual void ReadStdOut()
        {
            try
            {
                while (true)
                {
                    ...
                    int bytesRead = process.StandardOutput.BaseStream.Read(buffer, 0, 256);// .BeginRead(buffer, 0, 256, null, null);
                    if (bytesRead > 0)
                    {
                        FireAsync(BytesOutReceived, this, new BytesReceivedEventArgs(buffer, bytesRead));
                    }
                    else
                    {
                        process.WaitForExit();
        ...
        protected virtual void ReadStdErr()
        {
            try
            {
                ...
                {
                    byte[] buffer = new byte[256];
                    int bytesRead = process.StandardError.BaseStream.Read(buffer, 0, 256);
                    if (bytesRead > 0)
                    {
                        FireAsync(BytesOutReceived, this, new BytesReceivedEventArgs(buffer, bytesRead));
                    }
